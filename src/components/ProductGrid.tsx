...
import { ShoppingCart, Star, ArrowRight, Heart, Info, ChevronLeft } from 'lucide-react';
...
function CascadeImages({ images }: { images: string[] }) {
  // Tomar hasta 4 imágenes, filtrar las vacías
  ...
  // Posiciones de la cascada según la cantidad de imágenes
  // La imagen más reciente (último índice) está al frente (bottom-left)
  // La más antigua (índice 0) está al fondo (top-right)
...
export function ProductGridCard({ product, onAddToCart }: { product: Product; onAddToCart?: (p: Product, quantity?: number) => void }) {
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-col flex-1 px-1 md:px-2">
          ...
          <h3 className="text-[11px] md:text-base font-display font-bold text-[#1E1B4B] line-clamp-2 leading-tight mb-1 group-hover:text-[#1A8A00] transition-colors">
            {formatProductName(product)}
          </h3>
          <p className="text-[10px] text-gray-400 font-mono mt-0.5">{product.product_code}</p>
          ...
          <div className="mt-auto pt-1 md:pt-2">
            ...
            <div className="flex items-center justify-between flex-wrap gap-y-2 gap-x-1 md:gap-2">
              ...
              {onAddToCart && (displayProduct.stock === undefined || displayProduct.stock === null || displayProduct.stock > 0) ? (
                <div className="flex items-center flex-wrap justify-end gap-2 md:gap-2.5">
                  <div className="flex items-center gap-0.5 md:gap-1 border border-gray-200 rounded-full bg-gray-50 px-1 py-0.5">
                    <button onClick={(e) => { e.stopPropagation(); setQty(q => Math.max(1, q - 1)); }}
                      className="w-5 h-5 md:w-6 md:h-6 flex items-center justify-center text-gray-500 hover:text-[#1A8A00] text-xs md:text-sm font-bold select-none">−</button>
                    <span className="w-4 md:w-5 text-center text-[10px] md:text-xs font-bold">{qty}</span>
                    <button onClick={(e) => { e.stopPropagation(); setQty(q => Math.min(displayProduct.stock ?? 99, q + 1)); }}
                      className="w-5 h-5 md:w-6 md:h-6 flex items-center justify-center text-gray-500 hover:text-[#1A8A00] text-xs md:text-sm font-bold select-none">+</button>
                  </div>
                  <button
                    onClick={(e) => {
  ...
  // Si ES padre: tarjeta con la misma altura base y popover para ver variantes
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col flex-1 px-1 md:px-2">
        ...
        <h3 className="text-[11px] md:text-base font-display font-bold text-[#1E1B4B] line-clamp-2 leading-tight mb-1 group-hover:text-indigo-600 transition-colors">
          {formatProductName(product)}
        </h3>
        <p className="text-[10px] text-gray-400 font-mono mt-0.5">{product.product_code}</p>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {onAddToCart && (
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  onAddToCart(variant, variantQty);
...
export default function ProductGrid({ onAddToCart, onViewProduct }: ProductGridProps) {
  ...
  const loadProducts = async () => {
    ...
    const { data } = await supabase
      .from('products')
      .select('id, name, public_name, price, special_price, differentiated_price, image_url, uploaded_image_url, stock, url_slug, product_code, is_parent, is_featured, category, parent_product_id, category_general, category_detail, is_bulk')
      .eq('active', true)
  ...
  if (loading) return (
    <div className="py-24 text-center">
      ...
      <p className="font-display font-bold text-[#1A8A00] text-xl">Cargando productos...</p>
    </div>
  );
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
          {products
            ...
            .map((product) => (
              <ProductGridCard key={product.id} product={product} onAddToCart={onAddToCart} />
            ))}
        </div>
        ...
        <div className="mt-16 text-center">
          <button
            ...
            className="inline-flex items-center gap-3 px-10 py-4 bg-[#1A8A00] text-white rounded-full hover:bg-[#064E3B] transition-all font-display font-bold text-base shadow-xl hover:shadow-2xl hover:-translate-y-1"
          >
